...
#define POSITION_DELTA_COUNT \
  (sizeof(mPositionDeltas) / sizeof(mPositionDeltas[0]))
...
float Wheel::getAngularSpeed() const {
  return mPositionDeltaSum * mUpdateFrequency * INCREMENTS_TO_RADIANS /
         POSITION_DELTA_COUNT;
}

void Wheel::setTargetSpeed(float speed) {
  mPositionDeltaSumTarget =
      speed * POSITION_DELTA_COUNT / INCREMENTS_TO_RADIANS / mUpdateFrequency;
  mPositionDeltaSumIntegratedError = 0;
  if (mBypassPID) mMotor.setPower(speed);
}

void Wheel::regulatePower() {
  if (mBypassPID) return;
  ...
  mPositionDeltaSumIntegratedError =
      mPositionDeltaSumIntegratedError > (127 << FIXED_POINT_SHIFT)
          ? (127 << FIXED_POINT_SHIFT)
          : mPositionDeltaSumIntegratedError < (-127 << FIXED_POINT_SHIFT)
                ? (-127 << FIXED_POINT_SHIFT)
                : mPositionDeltaSumIntegratedError;
  ...
  mMotor.setPower(output > 127 ? 127 : output < -127 ? -127 : output);
}
