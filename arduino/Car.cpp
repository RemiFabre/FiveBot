...
void Car::setupUpdateTimer() {
  // Setting timer1 with the following configuration: outputs A and B are used
  // with the minimum prescaler (1). The duty-cycle of the PWM A is controlled
  // by OCR1A (range [0, 255], motors 2 and 3, still noisy). The duty-cycle of
...
void Car::readCommand() {
  if (mCom.canRead()) {
    ...
    if (cmd == 's') {
      ...
      mCom.read(w);
      mCom.read(bypassPID);
      for (int i = 0; i < 4; ++i) mWheels[i].mBypassPID = bypassPID;
      setSpeed(vx, vy, w);
    }
    mCom.readEnd();
  }
...
ISR(TIMER2_COMPB_vect) { car->updateOdometry(); }
